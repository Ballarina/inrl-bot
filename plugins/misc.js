...
const text = message.client.text || message.quoted.text;
	    if (!text) return await client.sendMessage( message.from, { text: 'Enter A text to convert base64'}, { quoted: message });
           let encodedString = base64e(text);
            ...
            return await client.sendMessage( message.from, { text: encodedString }, { quoted: message } );
        }));
...
const text = message.client.text || message.quoted.text;
	    if (!text) return await client.sendMessage( message.from, { text: 'Enter A text to convert base64'}, { quoted: message });
           let decodedString = base64d(text);
            ...
            return await client.sendMessage( message.from, { text: decodedString }, { quoted: message } );
        }));
...
return await client.sendMessage( message.from, { text : ageOfYou},{ quoted: message })
});
